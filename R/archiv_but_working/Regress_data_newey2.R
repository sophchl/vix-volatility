...
Regress_data_newey2 <- function(Df_regress, SavePath1, SavePath2) {
  ...
  stargazer(MyRegression1a, MyRegression2a, MyRegression3a, se = list(Robust_se1a, Robust_se2a, Robust_se3a), t = list(Robust_t1a, Robust_t2a, Robust_t3a), p = list(Robust_p1a, Robust_p2a, Robust_p3a), add.lines=list(c("AIC", round(AIC(MyRegression1a),1), round(AIC(MyRegression2a),1), round(AIC(MyRegression3a),1))), dep.var.labels = c("Realized Volatility"), covariate.labels = c("c", "$\beta^{RV,d}_{t}$", "$\beta^{RV,w}_{t}$", "$\beta^{RV,m}_{t}$", "$\beta^{crisis}", "$\beta1{VIX,d}_{t}$", "$\beta^{VIX,w}_{t}$", "$\beta^{VIX,m}_{t}$"), omit.stat = "f", intercept.bottom = FALSE, intercept.top = TRUE, title = "level regression", out = SavePath1)
  stargazer(MyRegression1b, MyRegression2b, MyRegression3b, se = list(Robust_se1b, Robust_se2b, Robust_se3b), t = list(Robust_t1b, Robust_t2b, Robust_t3b), p = list(Robust_p1b, Robust_p2b, Robust_p3b), add.lines=list(c("AIC", round(AIC(MyRegression1b),1), round(AIC(MyRegression2b),1), round(AIC(MyRegression3b),1))), dep.var.labels = c("Realized Volatility"), covariate.labels = c("c", "$\beta^{RV,d}_{t}$", "$\beta^{RV,w}_{t}$", "$\beta^{RV,m}_{t}$", "$\beta^{crisis}", "$\beta1{VIX,d}_{t}$", "$\beta^{VIX,w}_{t}$", "$\beta^{VIX,m}_{t}$"), omit.stat = "f", intercept.bottom = FALSE, intercept.top = TRUE, title = "logarithmic regression", out = SavePath2)
  ...
  # not complete: column.labels = c("Reg1", ..) missing
}
